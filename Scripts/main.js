 ...
 * -------------------------------------------
 */
let RightTopPipe = Ground.extend({
	init: function(x, y, level) {
		let blocking = ground_blocking.all;
		this._super(x, y, blocking, level);
...
let RightPipe = Ground.extend({
	init: function(x, y, level) {
		let blocking = ground_blocking.right + ground_blocking.bottom;
		this._super(x, y, blocking, level);
		this.setImage(images.objects, 36, 390);
	},
}, 'pipe_right');
let LeftPipe = Ground.extend({
	init: function(x, y, level) {
		let blocking = ground_blocking.left + ground_blocking.bottom;
		this._super(x, y, blocking, level);
 ...
 * -------------------------------------------
 */
let RightPipeGrass = Decoration.extend({
	init: function(x, y, level) {
		this._super(x, y, level);
		this.setImage(images.objects, 36, 424);
	},
}, 'pipe_right_grass');
let LeftPipeGrass = Decoration.extend({
	init: function(x, y, level) {
		this._super(x, y, level);
		this.setImage(images.objects, 2, 424);
	},
}, 'pipe_left_grass');
let RightPipeSoil = Decoration.extend({
	init: function(x, y, level) {
		this._super(x, y, level);
		this.setImage(images.objects, 36, 458);
	},
}, 'pipe_right_soil');
let LeftPipeSoil = Decoration.extend({
	init: function(x, y, level) {
		this._super(x, y, level);
...
let Mushroom = ItemFigure.extend({
	init: function(x, y, level) {
		this._super(x, y, level);
		this.active = false;
		this.setSize(32, 32);
...
let Mario = Hero.extend({
	...
	setPosition: function(x, y) {
		...
		let w = (this.x <= 100) ? 0 : ((this.x >= this.level.width - 230) ? r : r / (this.level.width - 440) * (this.x - 210));		
	...
	crouch: function() {
		let coords = this.crouchSprites[this.state - 1][this.direction === directions.left ? 0 : 1];
		this.setImage(images.sprites, coords.x, coords.y);
		this.clearFrames();
	},
	jump: function() {
...
let GreenTurtle = Enemy.extend({
	...
	setVelocity: function(vx, vy) {
		this._super(vx, vy);
		let rewind = this.direction === directions.right;
		let coords = this.walkSprites[this.state - 1][rewind ? 1 : 0];
		let label = Math.sign(vx) + '-' + this.state;
